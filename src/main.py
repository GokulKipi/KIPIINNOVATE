...
# :white_check_mark: Fetch All Users (Fix: Removed `_id`)
@app.get("/users/", response_model=List[User])
async def get_users():
    users = list(users_collection.find({}, {"_id": 0}))  # Exclude `_id`
    return users
# :white_check_mark: Fetch All Ideas (Fix: Removed `_id`)
@app.get("/fetch_ideas/")
async def fetch_ideas():
    ideas = list(ideas_collection.find({}, {"_id": 0}))  # Exclude `_id`
    return clean_data(ideas) # :white_check_mark: Fix NaN and Infinity issues
# :white_check_mark: Assign an Idea to a Specific User (Fix: Using Pydantic Model)
...
# :white_check_mark: Create New User if Not Exists
...
async def get_user(email: str):
    ...
    return user
# :white_check_mark: Create New Idea and Assign a Random Reviewer
@app.post("/ideas/")
async def create_idea(idea: IdeaForm):
    ...
    # :hammer_and_wrench: Update the submitter's data and add 100 beans
    ...
    # :hammer_and_wrench: Debugging: Log the update result
...
# :white_check_mark: Fetch Idea by ID
...
def clean_data(data):
    ...
    if isinstance(data, dict):
        ...
        if math.isnan(data) or math.isinf(data):  # Handle NaN & Infinity
            return None  # Replace with None (or 0 if preferred)
    return data

# :white_check_mark: Fetch Review Ideas for a User
...
async def get_review_ideas(ids: str = Query(...)):
    ...
    return clean_data({"ideas": review_ideas})

# :white_check_mark: Update an Idea
