...
# Assign an Idea to a Specific User
...
async def create_idea(idea: IdeaForm):
    ...
    # Update the submitter's data and add 100 beans
...
# Update Likes for an Idea
class UpdateLikesRequest(BaseModel):
    idea_id: str
    likes: int

@app.post("/update_likes/")
async def update_likes(request: UpdateLikesRequest):
    idea = ideas_collection.find_one({"idea_id": request.idea_id})
    if not idea:
        raise HTTPException(status_code=404, detail="Idea not found")
    ideas_collection.update_one(
        {"idea_id": request.idea_id},
        {"$set": {"likes": request.likes}}
    )
